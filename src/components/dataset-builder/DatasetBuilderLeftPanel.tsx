...
// These represent competitors already added in the Competitors settings page
const WORKSPACE_COMPETITORS = [
...
export default function DatasetBuilderLeftPanel({ sources, onAddSource, onRemoveSource, activeFilters, onRemoveFilter, onClearAllFilters }: Props) {
  ...
  const addedIds = competitorSources.map(s => {
    // Extract original competitor id from source id pattern "comp-{id}-{timestamp}"
    const match = s.id.match(/^comp-(.+?)-\d+$/);
    return match ? match[1] : s.id;
  });

  const availableCompetitors = WORKSPACE_COMPETITORS.filter(c => !addedIds.includes(c.id));

  const handleAddCompetitor = (comp: typeof WORKSPACE_COMPETITORS[0]) => {
    onAddSource({
            ...
            <Popover open={popoverOpen} onOpenChange={setPopoverOpen}>
              ...
              <PopoverContent className="w-56 p-1.5" align="start">
                {availableCompetitors.length > 0 ? (
                  <div className="space-y-0.5">
                    {availableCompetitors.map(comp => (
                      <button
                        ...
                        className="w-full flex items-center gap-2.5 px-2.5 py-1.5 rounded-md hover:bg-muted transition-colors text-left"
                      >
                        <img
                          src={comp.avatar}
                          alt={comp.name}
                  ...
                  <p className="text-[11px] text-muted-foreground text-center py-3">All competitors added</p>
                )}
                <div className="border-t border-border mt-1.5 pt-1.5">
                  <button
                    onClick={() => { setPopoverOpen(false); navigate("/brand-data-room/competitors"); }}
                    className="w-full flex items-center gap-2 px-2.5 py-1.5 rounded-md hover:bg-muted transition-colors text-left text-[11px] text-muted-foreground hover:text-foreground"
                  >
